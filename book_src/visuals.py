...
BRIEFS = {
 ...
 '18-module': ('code','workbench','equation','y = xWᵀ + b','وزن و Bias باید به‌عنوان Parameter در Module ثبت شوند.'),
 ...
 '29-scores': ('attention','workbench','matrix','q₀k₀|q₀k₁|q₁k₀|q₁k₁','هر سطر مربوط به یک Query و هر ستون مربوط به یک Key است؛ هر خانه یک ضرب داخلی است.'),
 '30-scaling': ('attention','editorial','equation','scores = QKᵀ / √D','D تعداد ویژگی هر Head است، نه طول جمله.'),
 '31-values': ('attention','diagram','flow','scores|softmax|weights × V','به‌جای انتخاب یک Value، ترکیب وزن‌دار Valueهای مجاز را می‌سازیم.'),
 '32-self': ('attention','statement','branch','Q|K|V','در Self-Attention، هر سه تبدیل از همان دنباله آغاز می‌شوند.'),
 '33-mask': ('attention','case','mask','past|now|future','بالای قطر ممنوع است؛ Mask قبل از softmax اعمال می‌شود.'),
 ...
 '64-cache': ('research','diagram','equation','K,V = concat(cache, new)','Key و Value تازه به بخش ذخیره‌شده افزوده می‌شوند؛ سپس Attention محاسبه می‌شود.'),
 ...
 '65a-sft-lab': ('training','workbench','flow','base weights|response-only loss|held-out task','وزن‌های یکسان آغاز، داده و هدف متفاوت؛ ارزیابی وظیفه جدا از Loss است.'),
 '68-context': ('language','diagram','stack','instructions + question|selected evidence|reserved output','کل ورودیِ قالب‌بندی‌شده باید در بودجه جا شود؛ سهم پاسخ را پیشاپیش کنار می‌گذاریم.'),
 '69-chunks': ('language','workbench','flow','document|stable chunks|ranked evidence','شناسهٔ سند و محدودهٔ قطعه کمک می‌کنند دوباره سراغ شاهد اصلی برویم.'),
 '70-vectors': ('math','question','compare','shared vector space|cosine similarity','بردار شمارشی و Embedding آموخته یک قرارداد ندارند؛ شباهت تضمین حقیقت نیست.'),
 '71-grounding': ('debug','case','compare','citation exists|claim supported','وجود شناسهٔ منبع، به‌تنهایی پشتیبانی ادعای پاسخ را ثابت نمی‌کند.'),
 '72-history': ('sequence','statement','flow','turn 1|stored messages|next input','تاریخچه در برنامه است؛ مدل فقط بخشی را می‌بیند که دوباره در ورودی گذاشته‌ایم.'),
 '73-summary': ('sequence','editorial','compare','full conversation|lossy summary','خلاصه، متن مشتق‌شده و قابل خطاست؛ منبع و اطلاعات حذف‌شده مهم‌اند.'),
 '74-memory': ('code','workbench','stack','owner + consent|fact + provenance|update / delete','حافظهٔ پایدار، قرارداد نگهداری داده است؛ وزن مدل را تغییر نمی‌دهد.'),
 '75-tools': ('code','diagram','flow','request text|validate|dispatch|result','نوشتن نام تابع اجرای آن نیست؛ برنامه نام و آرگومان را می‌سنجد.'),
 '76-reasoning': ('math','case','flow','small steps|external calculation|independent check','گام‌های قابل بررسی برای حل مسئله‌اند؛ گزارش کامل محاسبهٔ درونی مدل نیستند.'),
 '77-candidates': ('research','question','compare','more candidates|measured correctness + cost','رأی بیشتر ممکن است خطای مشترک را تکرار کند؛ هزینه و درستی جدا سنجیده می‌شوند.'),
 '80-controller': ('architecture','diagram','flow','observe|propose|validate|act|verify|stop','هر مسیر باید به توقف برسد؛ سقف گام و ابزار بخشی از قرارداد سامانه است.'),
 '81-system-eval': ('debug','editorial','stack','retrieval|grounding|tool correctness|end-to-end','نمرهٔ یک جزء، موفقیت کل درخواست را تضمین نمی‌کند.'),
 '82-performance': ('training','workbench','compare','latency: seconds/request|throughput: tokens/second','بار کاری و دستگاه را ثابت نگه دارید؛ عددهای این دو معیار جای هم نیستند.'),
 '83-deployment': ('architecture','statement','flow','validated request|bounded worker|checked response','اجرای محلیِ محدود، جایگزین امنیت و پایش یک سرویس عمومی نیست.'),
 '84-capstone': ('model','diagram','stack','model + context|retrieval + memory|tools + verification','مدل یک جزء است؛ شواهد تحویل باید اتصال و محدودیت همهٔ جزءها را نشان دهند.'),
}
