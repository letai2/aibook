...
def stage_for(identifier):
    system_stages = {
        '65a-sft-lab': (23, 'تغییر رفتار با نمونهٔ پاسخ'),
        **dict.fromkeys(('68-context','67-rag','69-chunks','70-vectors','71-grounding'), (24, 'زمینه و بازیابی شاهد')),
        **dict.fromkeys(('72-history','73-summary','74-memory'), (25, 'تاریخچه و حافظهٔ بیرونی')),
        **dict.fromkeys(('75-tools','76-reasoning','77-candidates'), (26, 'ابزار و وارسی پاسخ')),
        **dict.fromkeys(('80-controller','81-system-eval'), (27, 'چرخهٔ محدود و ارزیابی')),
        **dict.fromkeys(('63-scale','64-cache','82-performance','83-deployment','84-capstone'), (28, 'هزینه و تحویل سامانه')),
    }
    if identifier in system_stages:
        return None, 'گسترش پروژه · ' + system_stages[identifier][1]
    ...
    for limit, stage, title in [(4,0,'مدل شمارشی'),(24,2,'متن و شناسه'),(25,3,'جدول نمایش'),(26,4,'اطلاعات موقعیت'),(32,5,'توجه'),(34,6,'Causal Mask'),(36,7,'Multi-Head Attention'),(37,8,'پیش‌خور'),(38,9,'مسیر جمع'),(39,10,'نرمال‌سازی'),(40,11,'یک بلوک'),(42,12,'چند بلوک'),(46,13,'خروجی مدل'),(47,14,'حلقهٔ آموزش'),(49,15,'سنجش آموزش'),(53,16,'ذخیره و ادامه'),(54,17,'تولید'),(55,19,'دما'),(56,21,'انتخاب نشانه'),(99,22,'مدل و پژوهش شخصی')]:
        if n <= limit:
            return stage, title
...
<ol class="pipeline">
<li><span class="node-tag">۰۱ · متن</span><b>مدلِ ما</b><small>کاراکتر‌ها → شناسه‌ها</small><code>(B,T)</code></li>
...
<figcaption>در مدل ما نرمال‌سازی پیش از هر زیرلایه است. پیکان میان‌بر، مقدار را بدون آن زیرلایه به جمع می‌رساند؛ مشکل‌های Gradient را به‌تنهایی برطرف نمی‌کند.</figcaption></figure>'''
...
<p>اینجا مانند Tokenizer پروژه، هر Code point از Unicode یک Token است؛ نه یک واژه و نه لزوماً یک کاراکتر که روی صفحه می‌بینیم. واژگان از متن مرجع ثابت زیر ساخته می‌شود و شناسهٔ صفر برای ناشناخته است. جدول نمایش سه‌ستونیِ بعدی فقط مثال دستی است، نه وزن آموزش‌دیده.</p>
...
def softmax_lab():
    return '''<section class="interactive" data-probability-lab><p class="lab-kicker">آزمایش ۰۲ / میز احتمال</p><h2>امتیاز را با اطمینان اشتباه نگیریم</h2><p>سه نشانهٔ فرضی داریم. امتیازهای خام را تغییر دهید؛ softmax از آن‌ها توزیع می‌سازد. هدف این مثال «الف» است. «دما» عدد مثبتی است که پیش از softmax امتیازها را بر آن تقسیم می‌کنیم؛ در درس تولید دوباره به آن برمی‌گردیم. نخست حدس بزنید دما چه اثری روی احتمال هدف دارد. Loss نمایش‌داده‌شده همان منفی لگاریتم این احتمال است؛ محاسبه‌اش را در درس بعد دنبال می‌کنیم.</p>
...
<div data-probability-bars class="probability-bars"></div><p class="readout" data-probability-result aria-live="polite"></p><p class="meta">دما در تولید به توزیع دست می‌زند؛ وزن شبکه را آموزش نمی‌دهد. کم‌شدن Loss با انتخاب هدف دلخواه، شاهد تعمیم نیست.</p></section>'''


def attention_lab():
    return '''<section class="interactive" data-attention-lab><p class="lab-kicker">آزمایش ۰۳ / اتاق توجه</p><h2>از یک پرسش تا یک جمع وزن‌دار</h2><p>مثال دستی با سه موقعیت و دو ویژگی؛ نه وزن‌های ذخیره‌شدهٔ مدل. هر سطر مربوط به یک Query و هر ستون مربوط به یک Key است. سطر سوم را انتخاب و پیش از تغییر Mask حدس بزنید کدام سطرها تغییر می‌کنند. «Causal Mask» یعنی منع دیدن موقعیت‌های بعدی؛ چند درس جلوتر آن را در کد می‌سازیم.</p>
...
<div class="control-row"><label><input type="checkbox" data-causal checked> Causal Mask روشن</label><label>سطر پرسش <select data-query><option value="0">۰ · نخستین</option><option value="1" selected>۱ · میانی</option><option value="2">۲ · آخرین</option></select></label></div>
<div class="matrix-pair"><div><h3>امتیاز مقیاس‌شده · QKᵀ / √2</h3><div data-attention-scores></div></div><div><h3>وزن پس از Mask و softmax</h3><div data-attention-weights></div></div></div><p class="readout" data-attention-result aria-live="polite"></p><p class="meta">Query، Key و Value خروجی سه Projection آموختنی‌اند، نه سه نوع کلمه. Parameterهای Projection آموخته می‌شوند؛ Q/K/V با ورودی تازه دوباره محاسبه می‌شوند. خانهٔ ممنوع پیش از softmax برابر منفی بی‌نهایت می‌شود؛ صفرکردن امتیاز کافی نیست.</p></section>'''


def shape_lab():
    return '''<section class="interactive" data-shape-lab><p class="lab-kicker">آزمایش ۰۴ / جابه‌جایی محورها</p><h2>عددها ثابت؛ قرارداد محور عوض می‌شود</h2><p>B تعداد نمونه، T طول زمینه، C ویژگی‌ها، H Headها و D ویژگی هر Head است. این ابزار شکل‌ها و هزینهٔ یک ماتریس امتیاز float32 را حساب می‌کند، نه حافظهٔ کل آموزش.</p>
...
<div data-shape-result class="shape-route" aria-live="polite"></div><p class="meta">در این مسیر، <code>reshape</code> ویژگی‌ها را میان Headها گروه‌بندی می‌کند و <code>transpose</code> جای محور زمان و Head را عوض می‌کند. اگر شکل خروجی درست باشد اما ترتیب عددها درست جابه‌جا نشده باشد، Multi-Head Attention محاسبهٔ موردنظر را انجام نمی‌دهد.</p></section>'''
...
<p>این میز برای پس از درس ردیابی مدل است. checkpoint یعنی فایل وضعیت ذخیره‌شدهٔ مدل. ابزار Python یک Forward pass انجام می‌دهد؛ یعنی با وزن‌های فعلی از ورودی به خروجی می‌رسد و عددها را در قالب متنی JSON صادر می‌کند. این بخش شبکه را در مرورگر اجرا نمی‌کند؛ وزن‌ها یا فایل <code>.pt</code> را اینجا وارد نکنید. نمونهٔ همراه از اجرای کوچک روی دادهٔ همین کتاب است و نشانهٔ کیفیت زبان عمومی نیست.</p>
...
<div data-inspect-output hidden><div class="source-stamp" data-inspect-source></div><div class="control-row"><label>موقعیت متن <select data-inspect-position></select></label><label>پنجرهٔ محاسبه <select data-inspect-view><option value="embeddings">نشانه و نمایش</option><option value="qkv">پرسش، کلید، مقدار</option><option value="scores">امتیاز خام و مقیاس‌شده</option><option value="weights">Mask و وزن توجه</option><option value="logits">امتیاز و احتمال خروجی</option><option value="generation">گام‌های تولید</option><option value="metrics">منحنی آموزش</option></select></label></div>
<div data-inspect-content></div></div><p class="meta">هر فایل یک Layer و یک Head منتخب را نشان می‌دهد؛ برای مقایسه دوباره با <code>--layer</code> و <code>--head</code> خروجی بگیرید. وزن Attention توضیح کامل علت پاسخ نیست. همهٔ بارگذاری‌ها محلی‌اند؛ داده‌ای فرستاده نمی‌شود.</p></section>'''
...
<label>مشاهده و شاهد <textarea name="observation" rows="4" maxlength="8000" placeholder="شکل، Loss، خروجی، پیام خطا؛ دقیقاً چه اتفاقی افتاد؟"></textarea></label>
