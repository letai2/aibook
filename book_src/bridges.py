...
"""<p>ورودی دوعددی <code>x=[1,2]</code> را داریم و می‌خواهیم پاسخی نزدیک هدف ۲ بسازیم. این بار به‌جای ضریب مشترکِ مثال نخست، هر ورودی ضریب خودش را دارد. همچنین می‌خواهیم بتوانیم حتی برای ورودی‌های صفر، خروجی غیرصفر بسازیم و در پایان اجازه ندهیم پیش‌بینی منفی شود. این سه تصمیم را به یک تابع کوچک تبدیل می‌کنیم، سپس می‌سنجیم یک گام آموزش آن را بهتر می‌کند یا نه. برای دنبال‌کردن همهٔ عددها هنوز به PyTorch نیاز نداریم.</p>
