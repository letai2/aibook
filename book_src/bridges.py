...
"""<p>در درس قبل جای هر عدد را شناختیم؛ اکنون با آن‌ها کار می‌کنیم. دو Vector داریم: <code>u=[1,2]</code> و <code>v=[3,-1]</code>. نماد u₀ همان خانهٔ <code>u[0]</code> است. زیرنویس شمارهٔ خانه است، نه توان؛ 2³ یعنی ۲×۲×۲. مساویِ ریاضی برابری دو مقدار را می‌گوید؛ در دستور Python مانند <code>u = [1,2]</code> نام را به مقدار نسبت می‌دهیم.</p>
...
<p>دام Python: جمع دو list معمولی آن‌ها را به هم می‌چسباند و <code>2*u</code> فهرست را تکرار می‌کند. برای حساب عددی، اکنون حلقه می‌نویسیم و بعداً از Tensor استفاده می‌کنیم. برابر‌بودن Shape نیز کافی نیست: جمع ویژگی‌های «جرم و زمان» با «طول و قیمت» بدون قرارداد معنایی قابل توجیه نیست.</p>""",
...
"""<p>اکنون قطعه‌های یک گام یادگیری را بدون PyTorch کنار هم می‌گذاریم. <dfn>Neuron (یک واحد محاسباتی شبکه)</dfn> در این مثال ابتدا جمع وزن‌دار ورودی را می‌سازد؛ این نام به معنای شبیه‌سازی دقیق سلول زیستی نیست. <dfn>Weight (ضریب یک ویژگی ورودی)</dfn> اثر آن ویژگی را تنظیم می‌کند. <dfn>Bias (مقدار جابه‌جایی مستقل از ورودی)</dfn> اجازه می‌دهد خروجیِ پیش از Activation در ورودی صفر الزاماً صفر نباشد. هر دو Parameter قابل یادگیری‌اند.</p>
...
<p>Loss از ۱ به ۰٫۰۴ کم شد. این نتیجه برای همین گام است؛ هر Learning Rate چنین تضمینی ندارد. Backpropagation Gradient را محاسبه کرد و Gradient descent وزن‌ها را تغییر داد. این دو کار جدا هستند. تغییر Target، پیش‌بینی فعلی را عوض نمی‌کند اما Loss و Gradient را عوض می‌کند.</p>
