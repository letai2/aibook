...
<p>برای مسیر آزموده‌شده، نسخهٔ 3.11.x و عدد ۶۴ را انتظار داریم. اگر python نسخهٔ دیگری است یا Store را باز می‌کند، برای ساخت محیط از <code>py -3.11</code> استفاده کنید. پس از نصب، ترمینال را دوباره باز کنید. مسیر مفسر از عنوان پنجره مهم‌تر است.</p>
<h2>۳. وارد پوشهٔ پروژه شوید</h2>
<p>این مسیر نمونه را با محل واقعی خود عوض کنید. فاصله و نویسهٔ فارسی در نام پوشه مجاز است.</p>
...
python -c "import sys; print(sys.executable)"</code></pre>
<p>اگر PowerShell فعال‌سازی را مسدود کرد، سیاست امنیتی سراسری را تغییر ندهید؛ به‌جای python از <code>.\.venv\Scripts\python.exe</code> استفاده کنید. <code>deactivate</code> محیط فعال را ترک می‌کند.</p>
...
<h2>۷. رفع خطا</h2>
<ul><li>خطای import: مسیر Python نصب و اجرا را یکی کنید.</li><li>فایل data پیدا نشد: از ریشهٔ پروژه اجرا کنید، نه داخل mini_gpt.</li><li>پورت اشغال است: سرور قبلی خود را متوقف کنید یا پورت و URL را هر دو عوض کنید.</li><li>خروجی فارسی: CLIهای پروژه UTF-8 هستند. برای اسکریپت شخصی از <code>python -X utf8 lesson.py</code> استفاده کنید. برای JSON از گزینهٔ --output استفاده کنید؛ Windows PowerShell 5 ممکن است خروجی &gt; را با کدگذاری دیگری بنویسد.</li><li>CSS یا قلم دیده نمی‌شود: کل ZIP را استخراج کنید، نه فقط index.html را.</li><li>پوشهٔ اجرا غیرخالی است: نام تازه انتخاب کنید یا last.pt همان اجرا را Resume کنید؛ سابقه را برای رفع خطا پاک نکنید.</li></ul>
...
<p>بستهٔ <code>release/book-site.zip</code> فقط فایل‌های عمومی را دارد. مخزن، venv، archive و اجراهای مدل را منتشر نکنید. یک محیط مجازی تازه روی Windows موجود قوی‌تر از استفاده از نصب قبلی است، اما جای آزمون یک Windows تازه‌نصب‌شده را نمی‌گیرد. جزئیات شواهد و محدودیت‌ها در گزارش آموزشی مخزن ثبت می‌شود.</p><p>برای دفترهای تعاملی، [[notebooks.html|راهنمای Jupyter]] را دنبال کنید؛ همان محیط پروژه را به کار ببرید.</p>'''
