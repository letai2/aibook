...
''',
        task='تابع `encode(value, mapping)` فهرست شناسه‌ها و `decode(ids, vocabulary)` رشته را برگرداند. کاراکترِ ناشناخته باید همان `KeyError` طبیعی دیکشنری را بدهد؛ فعلاً نشانهٔ ناشناخته اضافه نکنید.',
...
''',
        vary='فقط فاصله را با نیم‌فاصله جایگزین کنید. با دیدن `repr` و تعداد کاراکترها توضیح دهید چرا شباهت دیداری، یکسان‌بودن رشته نیست.',
...
''',
        ...
        takeaway='کدام آزمون نشان می‌دهد IDها به همان کاراکترِ قبلی اشاره می‌کنند، نه فقط به یک کاراکترِ مجاز؟'),
...
''',
        vary='فقط فاصلهٔ دو امتیاز را تغییر دهید: `[0,gap]` برای `gap`های صفر، یک و دو. ترتیب ثابت است، اما اطمینان چطور تغییر می‌کند؟',
...
''',
    ...
    '10-entropy': dict(
        ...
        prerequisite='`08-probability` و `09-softmax`؛ این آزمایش دربارهٔ Cross Entropy با هدف تک‌کلاسه است.',
...
''',
        debug='میانگین دو Loss مربوط به Batchهای ۸ و ۲هدفه، به همهٔ هدف‌ها وزن برابر نمی‌دهد. تابع `aggregate_loss` جفت‌های `(mean_loss, count)` را درست ترکیب کند؛ `count`ها مثبت‌اند.',
...
''',
    ...
    '12b-neuron': dict(
        ...
        prerequisite='جمع وزن‌دار، `12-chain` و `12-sgd`؛ فقط Python.',
...
''',
        task='تابع `neuron(inputs,weights,bias)` `tuple`ِ `(z, prediction)` برگرداند: `z` جمع وزن‌دار همراه Bias و `prediction` برابر ReLU آن است. اندازه‌های نابرابر را رد کنید.',
...
EXERCISES.update({
    '21-tokenizer': dict(
        title='کاراکترِ ندیده و واژگانِ ذخیره‌شده',
        goal='قرارداد Unknown token را پیاده و با Tokenizer واقعی مقایسه کنید.',
        prerequisite='`02-token` و `21-tokenizer`؛ JSON و Path فقط در نمایش ذخیره‌سازی استفاده می‌شوند.',
...
print("Vocabulary:", list(enumerate(vocabulary)))
''',
        task='تابع `encode_unknown(text,vocabulary)` فهرست IDها را بسازد. فهرست Vocabulary معتبر است و خانهٔ صفر آن `<|unk|>` است. هر کاراکتری را که در Vocabulary نیست به صفر ببرید؛ ترتیب فهرست را حفظ کنید.',
...
''',
    ...
    '22-bpe': dict(
        title='یک ادغام، بدون حذف یا تکرار کاراکترها',
...
''',
    ...
    '24-data-contract': dict(
        ...
        prerequisite='`21-tokenizer` و `23-shift`؛ شمارش شناسه‌ها و فایل UTF-8.',
        predict='اگر کاراکترِ `Z` فقط در انتهای اعتبارسنجی باشد، آیا باید وارد Vocabulary آموزش شود؟ آیا طول غیرصفر برای ساخت پنجره کافی است؟',
        setup='''from pathlib import Path
from tempfile import TemporaryDirectory
...
''',
        vary='فقط یک کاراکتر از محتوای فایل موقت را عوض کنید. اثر انگشت باید عوض شود؛ این تغییر چیزی دربارهٔ بهتر یا بدترشدن کیفیت متن نمی‌گوید.',
...
''',
    ...
    '25-embedding': dict(
        ...
        prerequisite='`17-autograd`، `18-module` و `21-tokenizer`؛ Embedding در درس جاری.',
...
''',
    ...
    '26-positions': dict(
        ...
        prerequisite='`15-broadcast` و `25-embedding`؛ هنوز ساخت Attention لازم نیست.',
        predict='دو وقوع ID یک، قبل و بعد از افزودن موقعیت چه رابطه‌ای دارند؟ آیا جمع دو Vector `C`تایی خروجی `2C`تایی می‌دهد؟',
        setup='''import torch
from torch import nn
...
''',
        task='تابع `ids_tensor(rows)` جدول مستطیلی شناسه‌ها را به Tensor با نوع `torch.long` و روی CPU تبدیل کند. `rows` در این تمرین شامل `int` است.',
...
''',
    ...
    '15-broadcast': dict(
        ...
        prerequisite='`13-torch` و `14-index-device`؛ `mean`، `dim` و `keepdim` در درس.',
...
''',
    ...
    '16-reshape': dict(
        ...
        prerequisite='درس‌های ۱۳ تا ۱۵؛ `reshape`، `transpose` و `squeeze` در درس جاری.',
...
''',
        task='تابع `inspect_gradient(value)` Tensor تک‌عنصری تازه با `requires_grad=True` بسازد، `Loss=(2w-5)²` را `backward` کند و `tuple`ِ عددی `(loss, gradient, unchanged_weight)` برگرداند. هیچ به‌روزرسانی انجام ندهید.',
...
''',
    ...
    '19-network': dict(
        ...
        prerequisite='`17-autograd` و `18-module`؛ Cross Entropy و Adam در درس جاری.',
...
''',
        task='`build_network(width)` شبکهٔ `Linear(2,width)`، Tanh و `Linear(width,2)` بسازد. `train_step(model,optimizer,x,targets)` گرادیان قبلی را پاک کند، Cross-Entropy را مستقیماً از Logits محاسبه و `backward` کند و یک `step` انجام دهد؛ Loss پیش از به‌روزرسانی را به‌صورت `float` برگرداند.',
...
''',
        debug='در قرارداد هدف تک‌کلاسه، شناسه‌ها باید `long` باشند. تابع `class_ids` فقط فهرست `int`های نامنفی را بپذیرد و Tensor نوع `long` برگرداند؛ `float` را با گردکردن پنهان نکنید.',
        bug_code='''try:
...
''',
    ...
    '20-loader': dict(
        title='پنج نمونه و سه Batch؛ چرا آخری کوچک‌تر است؟',
        goal='Dataset را خودتان بسازید و پوشش همهٔ نمونه‌ها را بررسی کنید.',
        prerequisite='`14-index-device` و `19-network`؛ Dataset و DataLoader درس جاری.',
...
''',
        debug='حلقهٔ خراب با تقسیم صحیح، آخرین دسته را فراموش می‌کند. تابع `batch_ranges(n,size)` جفت‌های `(start,stop)` بسازد تا هر اندیس دقیقاً یک بار پوشش داده شود؛ `size` مثبت است.',
