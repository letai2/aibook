...
EXERCISES = {
    ...
    "03_tensor_shapes": _lab(
        title="میانگین هر بردار را کم کنید، سپس محورهای Batch و زمان را ادغام کنید",
        ...
        predict="اگر از هر بردار `C`تایی میانگین خودش را کم کنیم، ردیف `b*T+t` در خروجی صاف‌شده متعلق به کدام موقعیت است؟",
        setup="batch = torch.tensor([[[1.,2.,6.],[10.,20.,30.]],[[0.,3.,9.],[-2.,1.,4.]]])\nprint('B,T,C:',batch.shape)",
        task="تابع `center_and_pack(x)` ابتدا میانگین هر بردارِ محور آخر را از همان بردار کم کند، سپس `(B,T,C)` را به `(B*T,C)` ببرد. ترتیب نمونه و زمان حفظ شود.",
    ...
    "04_gradients_steps": _lab(
        ...
        takeaway="چرا مقایسهٔ دو Learning rate بدون ثابت‌نگه‌داشتن روش جمع‌زدن یا میانگین‌گیری Lossها می‌تواند گمراه‌کننده باشد؟",
    ),
    ...
    "10_block_trace": _lab(
        ...
        debug="مقایسهٔ دو بلوکِ تازه، اثر ترتیب را با تفاوت وزن‌ها مخلوط می‌کند. تابع `align_weights(reference,candidate)` مقدار `state_dict` شیء `reference` را در `candidate` بارگذاری و `candidate` را برگرداند؛ دو شیء همچنان مستقل بمانند.",
        ...
        connection="`TransformerBlock` پروژه Pre-Norm باقی می‌ماند. آزمایش مقایسه‌ای از همان قطعه‌ها استفاده می‌کند تا نام معماری به محل دقیق عمل‌ها وصل شود؛ پیاده‌سازی اصلی پروژه تغییری نمی‌کند.",
