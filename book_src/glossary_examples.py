...
FOCUSED_EXAMPLES = {
    'resume': {'meaning': 'ادامهٔ آموزش از وضعیت ذخیره‌شده است؛ برای ادامهٔ همان اجرا، علاوه بر وزن‌ها باید وضعیت Optimizer، شمارهٔ گام و مولدهای تصادفی هم بازیابی شوند.'},
    'autoregressive-generation': {'meaning': 'تولید پیاپی Tokenهاست: هر Token انتخاب‌شده به ورودی اضافه می‌شود تا مدل Token بعدی را پیش‌بینی کند.'},
    'base-language-model': {'meaning': 'مدل زبانیِ حاصل از Pretraining است، پیش از تنظیم آن برای دستورپذیری یا رفتار دستیار.'},
    'perplexity': {'meaning': 'در قرارداد این کتاب، exp میانگین Cross-Entropy روی Tokenهای هدف است؛ برای مقایسه باید داده و Tokenizer ثابت باشند.'},
    'learning-rate-schedule': {'meaning': 'قاعده‌ای است که نرخ یادگیری را بر اساس شمارهٔ گام آموزش تعیین می‌کند.'},
    'rank': {'meaning': 'تعداد سطرها یا ستون‌های مستقلِ خطیِ یک ماتریس است.'},
    'identity': {'meaning': 'تبدیلی است که ورودی را بدون تغییر برمی‌گرداند.'},
    'instruction-following-model': {'meaning': 'مدلی است که برای انجام درخواستِ ورودی آموزش دیده، نه صرفاً ادامه‌دادن متن؛ این نام تضمین درستی همهٔ پاسخ‌ها نیست.'},
    'reproducibility': {'meaning': 'یعنی بتوان آزمایش را با کد، داده و تنظیمات ثبت‌شده دوباره اجرا و نتیجه‌اش را بررسی کرد. برابری عددی دقیق به محیط و عملیات مورد استفاده هم وابسته است.'},
    'teacher-forcing': {'meaning': 'دادن پیشوند مرجع به مدل هنگام Training یا ارزیابی، به‌جای استفاده از خروجیِ تولیدشدهٔ قبلی آن است.'},
    'weight-decay': {
        'meaning': 'Weight Decay در قاعدهٔ به‌روزرسانی، وزن‌ها را به سمت صفر می‌کشد. در AdamW این کاهش از محاسبهٔ Gradient مربوط به Loss جداست؛ با کوچک‌کردن Learning Rate یکی نیست.',
        'intuition': 'مدل ممکن است با وزن‌های بزرگ دادهٔ آموزش را خوب حفظ کند، اما روی دادهٔ تازه ضعیف باشد. Weight Decay یکی از تنظیم‌هایی است که اثرش را با Validation می‌سنجیم؛ تضمین تعمیم نیست.'},
    ...
    'probability-distribution': {'example': "برای Vocabulary فرضیِ <code>['a','b',' ']</code>، احتمال‌های [0.2,0.3,0.5] یک توزیع معتبرند؛ هر احتمال به Token مشخص خودش مربوط است و مجموع یک است."},
